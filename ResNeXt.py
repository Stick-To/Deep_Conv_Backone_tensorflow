...
class ResNeXt:
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('split'):
            ...
            for i in range(self.block_list[0]):
                residual_block = self._residual_bottleneck(residual_block, self.filters_list[0], 1, 'block1_unit'+str(i+1))
            for i in range(1, len(self.block_list) - 1):
                residual_block = self._residual_bottleneck(residual_block, self.filters_list[i], 2, 'block'+str(i+1)+'_unit'+str(1))
                for j in range(1, self.block_list[i]):
                    residual_block = self._residual_bottleneck(residual_block, self.filters_list[i], 1, 'block'+str(i+1)+'_unit'+str(j+1))

            residual_block = self._residual_bottleneck(residual_block, self.filters_list[-1], 2, 'block'+str(len(self.block_list))+'_unit'+str(1))
    ...
    def _residual_bottleneck(self, bottom, filters, strides, scope, activation=tf.nn.relu):
